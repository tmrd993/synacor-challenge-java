...
public class Add implements Instruction {
	...
	int additionResult = Math.floorMod(paramB + paramC, VIRTUAL_MACHINE.getVirtualMachineMemory().getTotalMemorySize());
	VIRTUAL_MACHINE.getVirtualMachineMemory().setRegister(destination, additionResult);
    }
	...
	return List.of(memory.getFromMainMemory(INSTRUCTION_POINTER + 1),
		...
		memory.getFromMainMemory(INSTRUCTION_POINTER + 3));
    }
